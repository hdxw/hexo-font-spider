    ...
    if(html_list.length > 0){
        fontSpider.spider(html_list, {
            ...
            resourceMap: function(file) {
                ...
                let origin_url = path.resolve(root_dir).replaceAll(path.win32.sep, path.posix.sep).toLowerCase();
                let actual_url = path.join(process.cwd(), html_dir).replaceAll(path.win32.sep, path.posix.sep).toLowerCase();
